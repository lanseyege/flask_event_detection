...
class Event_Stream:
    ...
    def __init__(self, clusters_num, one_clusters_num):
        ...
        self.tweetss = []
        self.one_cluster_tweet_nums = one_clusters_num

    def get_tweets(self, tweet, inx):
        #tweet = ' '.join(l for l in tweet.strip().split())
        self.tweetss.append(tweet)
        self.get_one_tweet(inx, tweet)
        ps1 = []
        for ret in self.clusters_return:
            print('ret')
            print(ret)
            ps2 = []
            if len(ret) == 0:
                ...
                for re in ret:
                    ps2.append(self.tweetss[re])
                if len(ret) < 2:
                    ps2.append('')
            ps1.append(ps2)
        return ps1, self.index_
    ...
    def get_cluster(self, words_em, inx):
        ...
        if max_v < 1.25 * sum(self.cosines) / self.clusters_num:
            ...
            if self.all < self.clusters_num:
                ...
                k.append(words_em)
                if len(k) > self.one_cluster_tweet_nums:
                    k = k[len(k) - self.one_cluster_tweet_nums:]
